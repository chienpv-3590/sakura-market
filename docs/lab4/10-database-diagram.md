...
| D-08 | Đường "Gỡ tạm ngừng" | **RFP FIG-004 (dòng 290-294)** đòi cột "Gỡ tạm ngừng" **khác nhau theo loại**: 卸売業者 → Chấp thuận · 仲卸 → **Có điều kiện** · 売買参加者 → **Xét lại**. Ba đường khác nhau, không phải một | **Một** transition duy nhất `{ from: "tạm ngừng", event: "go", to: "có hiệu lực" }` dùng chung cho **cả 4 loại**. Route xử lý transition **không đọc `participant.category`** một lần nào | `src/lib/participants/state-machine.ts:37` (cạnh gộp), `:35-41` (đủ 5 cạnh); `src/app/api/participants/[id]/transition/route.ts:45-49` chỉ `select("status")`, không select `category`; `:58` gọi `resolveTarget(from, event)` — chữ ký chỉ nhận `(from, event)`, không có chỗ nào truyền `category` vào (`state-machine.ts:65-71`) | **khác không chủ đích — divergence nặng nhất của cả bộ nộp** | Đây đúng là cái RFP §02-08 dòng 309 cấm: gộp 許可 và 承認 thành một quy tắc chung. Ba khác biệt bị xoá phẳng: (a) 仲卸 phải gỡ "có điều kiện" — không có chỗ nào lưu điều kiện là gì, ai đặt, khi nào hết; (b) 売買参加者 phải qua "Xét lại", tức đi `tạm ngừng → xét lại → có hiệu lực`, nhưng `state-machine.ts:39` chỉ cho vào `xét lại` từ `mất hiệu lực`, nên đường đúng của 売買参加者 **không tồn tại**; (c) 卸売業者 gỡ bằng "Chấp thuận" — cần người duyệt khác người yêu cầu, hiện không có maker-checker nào ở luồng transition. **Cách sửa:** `resolveTarget` nhận thêm `category`; `TRANSITIONS` tách thành bảng theo `(category, from, event)`; thêm cạnh `tạm ngừng → xét lại` cho 売買参加者; thêm bảng/cột lưu điều kiện gỡ của 仲卸; thêm maker-checker cho nhánh "Chấp thuận". Đây là thay đổi cả DB lẫn app, không phải sửa một hằng số — nên phải là ADR riêng ở phase-07 và là task riêng ở LAB-5, không gộp vào "sửa nhỏ" |
...
quyết định **có chủ đích** của FR-601 và của nhu cầu review LAB-7. Dựng một màn duyệt hàng loạt lên
trên một bảng đọc rộng là chỗ hai yêu cầu đúng đắn đá nhau: FR-601 muốn mở, RFP §09-06 (APPI) đòi
kiểm soát truy cập theo vai trò cho thông tin cá nhân. **Không quyết trong tài liệu này.**
...
**Kéo theo:** spec SC-07 cũng đòi mã hoá ba đường "Gỡ tạm ngừng" của FIG-004 vào máy trạng thái —
đó đúng là § 4 dòng D-08, sửa ở `src/lib/participants/state-machine.ts`, không sửa ở màn này.
...
| `spec/SC-13`, `spec/SC-14` (nhánh せり) | § 4 dòng D-16 và D-17 → hai cửa kiểm thiếu và đường giao nhận thiếu; ảnh hưởng cả `spec/SC-11`, `SC-12` (tồn dùng chung) và `SC-18` (`variance` NULL) |
| **LAB-5, break task DB** | § 5.4 → 13 bảng mới · 10 cột thêm · 7 index, kèm cột "chỉ nếu" cho các hạng mục phụ thuộc quyết định |
| `adr/` (phase-07) | § 4 các dòng **khác không chủ đích**: D-06 `license_type` · D-07 `category` · **D-08 đường gỡ tạm ngừng** · D-13 `audit_log.action` · D-14 lịch sử trạng thái · D-15 append-only · **D-16 hai cửa kiểm của nhánh せり** · D-17 đường giao nhận của nhánh せり · D-18 index của `audit_log`. Cộng danh sách quyết định-cần-ADR ở cuối § 5.4 |
